...
class RecipeView extends view {
  ...
  errorMessage = `We could not find that recipe. Try another one, bich😝`;
  message = '';
  ...
  addHandlerServings(handler) {
    this.parentElement.addEventListener('click', function (e) {
      ...
      if (!btn) return;
      let updateTo = Number(btn.dataset.updateTo);

      console.log(btn);
      // we need to call handler function, or it will not work
      if (updateTo > 0) {
        handler(updateTo);
      }
    });
  ...
  <div class="recipe__ingredients">
    ...
    ${this.data.ingredients
      .map(ing => {
        return this.generateIngredients(ing);
      })
      .join(' ')}
